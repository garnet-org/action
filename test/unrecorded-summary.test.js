...
test("a missing profile renders the no_profile line and nothing else", () => {
    const summary = renderUnrecordedSummary("No execution profile was produced for this job (status: no_profile)")
    assert.ok(summary.includes("No execution profile was produced for this job (status: no_profile)"))
    assert.doesNotMatch(summary, /no change|clean|egress/i)
})
